...
def spec_gating(clean_audio_path, noise_audio_path, noise_scaling_factor=1):
...
# Runs spectral gating with noisereduce on the first "num_processed_files" of clean audio samples
# Randomly chooses a noise sample for each of the clean audio samples
# Calculates average Signal to Noise Ratio
# Commented out lines in spec_gating function to visualize spectrograms and output denoised audio files
total_snr = 0
for clean_file, noise_file in zip(clean_audio_files, noise_audio_files):
    ...
    snr = spec_gating(clean_path, noise_path)
    total_snr += snr
